...
export async function generateBusinessInsights(
  ...
  const [
    ...
    prisma.expense.aggregate({ where: { businessId, date: { gte: thisM.start, lte: thisM.end } }, _sum: { amount: true } }),
    prisma.expense.aggregate({ where: { businessId, date: { gte: prevM.start, lte: prevM.end } }, _sum: { amount: true } }),
