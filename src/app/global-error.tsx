...
export default function GlobalError({
  ...
  return (
    <html lang={isRTL ? 'ur' : 'en'} dir={isRTL ? 'rtl' : 'ltr'} className={isRTL ? 'lang-ur' : undefined}>
      <body>
          ...
          <div className="max-w-md w-full text-center space-y-6 bg-white p-8 rounded-3xl border border-gray-200/80 shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 justify-center">
              ...
              >
                ...
                {t('staticPages.returnToDashboard')}
              </Link>
            </div>

            {error?.digest && (
              <div className="pt-4 border-t border-gray-100">
                <p className="text-xs text-gray-400 font-mono">{t('staticPages.referenceId', { id: error.digest })}</p>
              </div>
            )}
          </div>
        </div>
