    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="text-gray-900 font-semibold">{t('reports.yearlyReport')}</span>
        </div>
        ...
        <form method="GET" className="flex items-center gap-2">
          ...
          <input
            ...
            className="w-28 px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary"
          />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-xs font-semibold text-purple-600 uppercase tracking-wider">
            {t('reports.annualFinancialAudit')}
          </span>
          <h1 className="text-2xl font-bold text-gray-900 mt-0.5">
            {t('reports.yearFinancialStatement', { year })}
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            {t('reports.annualAuditDesc')}
          </p>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 p-3 rounded-xl flex items-center gap-3">
          ...
          <div>
            <span className="text-[11px] text-gray-400 font-semibold block">{t('reports.vsLastYear')}</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs">
          <span className="text-xs font-semibold text-gray-500 uppercase">{t('reports.annualRevenue')}</span>
          <h3 className="text-2xl font-bold text-gray-900 mt-1">
            {formatCurrency(summary.grossRevenue)}
          </h3>
          <p className="text-xs text-gray-400 mt-1">{t('reports.totalOrdersCaption', { count: formatNumber(summary.ordersCount) })}</p>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs">
          <span className="text-xs font-semibold text-emerald-700 uppercase">{t('reports.annualGrossProfit')}</span>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs">
          <span className="text-xs font-semibold text-red-700 uppercase">{t('reports.annualExpenses')}</span>
          <h3 className="text-2xl font-bold text-red-700 mt-1">
            {formatCurrency(summary.expenses)}
          </h3>
          <p className="text-xs text-red-500 mt-1">{t('reports.totalOperatingCosts')}</p>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs">
          <span className="text-xs font-semibold text-gray-950 uppercase">{t('reports.annualNetProfit')}</span>
          <h3 className="text-2xl font-bold text-gray-950 mt-1">
            {formatCurrency(summary.netProfit)}
          </h3>
          <p className="text-xs text-gray-800 mt-1">{t('reports.totalRetainedMargin')}</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
        <h3 className="font-bold text-gray-900 text-base">{t('reports.annualTrend')}</h3>
        <SimpleBarChart
          data={chartData}
          height={240}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-200">
          <h3 className="font-bold text-gray-900 text-base">{t('reports.monthlyBreakdownYear', { year })}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-start border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b">
                <th className="px-5 py-3.5 font-medium">{t('reports.month')}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {months.map((m) => (
                <tr key={m.month} className="hover:bg-gray-50/50">
                  <td className="px-5 py-3.5 font-semibold text-gray-900">{monthName(m.month - 1)}</td>
                  <td className="px-5 py-3.5 text-center font-medium text-gray-700">{formatNumber(m.orders)}</td>
